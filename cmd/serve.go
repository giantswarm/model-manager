...
environment variable named next to it; flags win over the environment.`,
	...
	f.StringVar(&k.budgetSource, "kserve-budget-source", envOr("KSERVE_BUDGET_SOURCE", kserve.DefaultBudgetSource), "Node memory budget: auto (GPU labels when present, else allocatable memory), gpu-labels, allocatable (KSERVE_BUDGET_SOURCE)")
