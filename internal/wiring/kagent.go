...
type ModelConfigRef struct {
	...
	// Accepted) and resolved what it references (condition ResolvedRefs: the
	// API-key Secret exists and holds the key); a status without ResolvedRefs
	// counts as resolved.
...
// Accepted must be True and ResolvedRefs must not be False — kagent reports a
// missing or incomplete Secret there while Accepted stays True. The message is
// the failing condition's, or Accepted's when nothing fails.
func readiness(conds []any) (ready bool, message string) {
	...
	for _, c := range conds {
		...
		case resolvedRefsCondition:
			if cm["status"] == "False" {
				unresolved, unresolvedMsg = true, msg
			}
		}
	}
	switch {
