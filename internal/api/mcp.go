...
func NewMCPServer(svc *service.Service, build buildinfo.Info, opts ...Option) *mcpserver.MCPServer {
	...
	s.AddTool(mcp.NewTool(ToolListPresets,
		mcp.WithDescription("List the curated serving presets (kserve): model, runtime, GPUs, the share of each GPU's memory vLLM claims (gpuMemoryUtilization), weights and overhead requirements, arguments. Presets are the only way to serve a model on kserve. A preset states no API interfaces: the ones a served model answers are read from its running server (list_loaded_models interfaces)."),
