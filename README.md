...
- **Phases** — `phase` refines `status` with where a serve is, and `steps`
  ...
  | `ready` | the model answered one request on its workload Service: a chat completion of one token (a pooling model: one embedding), sent once per Ready transition | `since` = the `Ready` condition's `lastTransitionTime`, `finishedAt` = the answer, `message: answered POST /v1/chat/completions`. A runtime that passed its readiness and answers that request with an error fails the step with `reason: FirstRequestFailed` and the runtime's status and first error line (`…answered 500 Internal Server Error: openai_harmony.HarmonyError: …`): the phase is `failed`, `status` `NotReady`, and `load_model` returns the error instead of waiting. A server whose route list names nothing to ask is not asked; the step says why |
