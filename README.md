...
- **Phases** — `phase` refines `status` with where a serve is, and `steps`
  ...
  | `routing` | the pod is ready, KServe resolves the route; once the object is Ready, until the model answered its first request (`reason: AwaitingFirstAnswer`, the message says why no answer came; asked again after a minute) | `Ready=False` `HTTPRoutesNotReady`, then `Ready=True` |
  | `ready` | the model answered one request on its workload Service: a chat completion of one token (a pooling model: one embedding), sent once per Ready transition, never beside one still in flight (a list waits 15 s for it, the request runs on for up to 2 min) | `since` = the `Ready` condition's `lastTransitionTime`, `finishedAt` = the answer, `message: answered POST /v1/chat/completions`. A runtime that passed its readiness and answers that request with an error fails the step with `reason: FirstRequestFailed` and the runtime's status and first error line (`…answered 500 Internal Server Error: openai_harmony.HarmonyError: …`): the phase is `failed`, `status` `NotReady`, and `load_model` returns the error instead of waiting. The failure stands while the object drops out of Ready (a runtime that hangs on the request fails its readiness too), until a later Ready transition's request is answered. A server whose route list names nothing to ask is not asked; the step says why |
